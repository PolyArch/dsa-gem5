...
void port_data_t::reformat_in_work() {
  ...
  for (unsigned cgra_port = 0; cgra_port < _port_map.size(); ++cgra_port) {
    for (unsigned i = 0; i < _port_map[cgra_port].second.size(); ++i) {
      ...
      // pull from mem data and push into cgra_port (mem_data filled in by
      // memory) SBDT val = _mem_data[ind];
...
template <typename T> T port_data_t::pop_in_custom_data() {
  assert(_mem_data.size());
...
void port_data_t::reformat_out_one_vec() {
  if (can_output() && _mem_data.size() + port_vec_elem() <= VP_LEN) {
    ...
    // cout << "first: " << _cgra_data[0].size() << " and num_ready: " <<
    // _num_ready << "\n";
...
void port_data_t::reformat_out_work() {
  ...
  for(unsigned cgra_port = 0; cgra_port < _port_map.size(); ++cgra_port) {
    for(unsigned i = 0; i < _port_map[cgra_port].second.size(); ++i) {
      unsigned ind = _port_map[cgra_port].second[i];
      _mem_data[cur_mem_data_pos + ind] = _cgra_data[cgra_port].front(); //first
element assert(_cgra_data[cgra_port].size() > 0);
...
void port_data_t::reformat_out_work() {
  ...
  for (unsigned cgra_port = 0; cgra_port < _port_map.size(); ++cgra_port) {
    for (unsigned i = 0; i < _port_map[cgra_port].second.size(); ++i) {
      ...
      // if(valid) push_data(get_custom_val(v,v.size())); // don't push back if
      // not valid
      if (valid)
        push_data(v, v.size());
      // if(valid) push_data(val); // don't push back if not valid
...
// ----------------------------- Port Interface
// ------------------------------------ void port_interf_t::initialize(SSModel*
// ssconfig, Schedule* sched) {
void port_interf_t::initialize(SSModel *ssconfig) {
  ...
  for (auto &x : ssconfig->subModel()->io_interf().in_vports) {
    ...
    // _in_port_data[port_index].initialize(ssconfig,port_index,true,vec_in->get_port_width());
    // // port, isInput, port_width
  }
...
uint64_t accel_t::receive(int out_port) {
  ...
  if (SS_DEBUG::COMMAND || SS_DEBUG::COMMAND_I || SS_DEBUG::COMMAND_O) {
    timestamp();
    cout << "SS_RECV  on port" << out_vp.port() << " " << out_vp.mem_size()
         << "\n";
  }
  return val;
}
...
void accel_t::cycle_cgra_backpressure() {
  ...
  for (int i = 0; i < active_out_ports.size(); ++i) {
    ...
    if (_dfg->can_pop_output(vec_output, len)) {
      ...
      // cout << "Allowed to pop output: " << data[0] << " and next output: " <<
      // data[1] << "\n"; cout << "Allowed to pop output: " << std::hex <<
      // data[0] << "\n";
...
/*
void scratch_write_controller_t::push_net_stream(remote_core_net_stream_t*
stream){ printf("TRYING TO PUSH IMPLICT STREAM\n");
  _accel->push_net_in_cmd_queue(stream);
}
*/

void accel_t::push_net_in_cmd_queue(base_stream_t *s) {
  printf("TRYUING TO PUSH IMPLICT STREAM IN CMD QUEUE\n");
...
{ assert(mask.size()==8); SBDT* u64data = (SBDT*)raw_data; for(int i = 0; i <
mask.size(); ++i) { if(mask[i]) { data.push_back(u64data[i]);
    }
  }
}
*/
...
/*
void apply_map(uint64_t* raw_data, const vector<int>& imap, std::vector<SBDT>&
data) {
  //SBDT* u64data = (SBDT*)raw_data;
...
void dma_controller_t::port_resp(unsigned cur_port) {
  if (Minor::LSQ::LSQRequestPtr response =
    ...
    if (_accel->_accel_index == response->sdInfo->which_accel) {
      ...
      // std::cout << "It was okay to push in values at the input ports? " <<
      // port_in_okay << "\n";
...
vector<uint8_t>
scratch_read_controller_t::read_scratch(affine_read_stream_t &stream,
                                        bool is_banked) {
  vector<uint8_t> data;
...
void network_controller_t::multicast_data(
  ...
  if (stream.stream_active() && out_vp.mem_size() >= message_size) {
    ...
    // _accel->_lsq->push_spu_req(remote_in_port, val, bytes_written,
    // stream._core_mask);
    // // num_bytes = message_size*sizeof(int64_t)
    // _accel->_lsq->push_spu_req(remote_in_port, val, message_size*8,
    // stream._core_mask);
    _accel->_lsq->push_spu_req(remote_in_port, val, remote_elem_sent * 8,
                               stream._core_mask);
  }
  if (_accel->_ssim->in_roi()) {
    // add_bw(stream.src(), stream.dest(), 1, bytes_written);
...
void network_controller_t::write_direct_remote_scr(
  ...
  if (stream.stream_active() && val_vp.mem_size() >= message_size) {
    ...
    int cores = 64;
    int dest_core_id =
        (final_scr_addr >> spad_offset_bits) & (cores - 1); // last bits?
    if (SS_DEBUG::NET_REQ) {
    ...
    // _accel->_lsq->push_spu_scr_wr_req(stream._scr_type, val, scr_offset,
    // dest_core_id, stream.id()); num_bytes = remote_elem_sent*8
    _accel->_lsq->push_spu_scr_wr_req(&val[0], remote_elem_sent * 8, scr_offset,
                                      dest_core_id, stream.id());
  }
  if (_accel->_ssim->in_roi()) {
    // add_bw(stream.src(), stream.dest(), 1, bytes_written);
...
void scratch_write_controller_t::cycle(bool can_perform_atomic_scr,
      ...
      if (stream.stream_active()) {
        ...
        if (out_vp.mem_size() > 0) {
          ...
          while (!_accel->isLinearSpad(addr) && addr < max_addr &&
            ...
            // variable
            // SBDT val = out_vp.pop_out_data();
            // uint8_t val = out_vp.pop_out_custom_data<uint8_t>(); // this is
            // te SBDT form of data
      ...
      if (stream.stream_active()) {
        ...
        if (out_addr.mem_size() > 0 && out_val.mem_size() > 0 &&
          ...
          while (scr_addr < max_addr && stream._num_strides > 0 &&
            ...
            // bank_id = (scr_addr >>
            // (sizeof(addr_t)*8-(int)(log(logical_banks)/log(2)))) &
            // (logical_banks-1); this is because scratchpad is byte addressable
            // and now we want it to be value width addressable bank_id =
            // (scr_addr >> (int)(log(SCRATCH_SIZE/logical_banks)/log(2))) &
            // (logical_banks-1); bank_id = (scr_addr >> 4) & (logical_banks-1);
            // bank_id = (scr_addr >> (int)(log(stream._value_bytes)/log(2))) &
            // (logical_banks-1);
      ...
      if (atomic_scr_issued_requests_active() && can_perform_atomic_scr) {
        for (int i = 0; i < logical_banks; ++i) {
          if (!_atomic_scr_issued_requests[i].empty()) {
            ...
            _accel->read_scratchpad(&input_val + 8 - request._value_bytes,
                                    scr_addr, request._value_bytes,
                                    stream.id());
            ...
            case 0:
              ...
              break;
            case 1:
              input_val = std::max(input_val, inc);
              break;
            case 2:
              input_val = std::min(input_val, inc);
              break;
            case 3:
              input_val = inc; // update
              break;
            default:
              cout << "Invalid opcode\n";
              break;
  ...
  if (_accel->_linear_spad) {
    ...
    for (unsigned i = 0; i < (_write_streams.size()); ++i) {
      ...
      _which_linear_wr = (_which_linear_wr + 1) >= (_write_streams.size())
                             ? 0
                             : _which_linear_wr + 1;
        ...
        if (stream.stream_active()) {
          ...
          if (out_vp.mem_size() > 0) {
            ...
            while (_accel->isLinearSpad(addr) && addr < max_addr &&
              ...
              if (SS_DEBUG::SCR_ACC) {
                // timestamp(); cout << "POPPED b/c port->linear scratch WRITE:
                // " << out_vp.port() << " " << out_vp.mem_size() << "\n";
                timestamp();
                cout << "POPPED b/c port->linear scratch WRITE: "
                     << stream._out_port
                     << " with width: " << stream._data_width << " "
                     << out_vp.mem_size() << "\n";
              }
...
void scratch_write_controller_t::push_remote_wr_req(int8_t *val, int num_bytes,
                                                    addr_t scr_addr) {
...
void port_controller_t::cycle() {
  ...
  for (unsigned i = 0; i < _port_port_streams.size(); ++i) {
    ...
    if (vp_out.mem_size() && port_in_okay) { // okay go for it
      ...
      uint64_t total_pushed = 0;
      for (int i = 0;
           i < PORT_WIDTH && vp_out.mem_size() && stream.stream_active();
           i += DATA_WIDTH) {
...
bool accel_t::done_concurrent(bool show, int mask) {
  ...
  // cout << "Line:3052 let's print 2 done: " << done << " and the other one: "
  // << cgra_done(show, mask) << endl;
...
void accel_t::configure(addr_t addr, int size, uint64_t *bits) {
  ...
  for (int ind = 0; ind < _dfg->num_vec_input(); ++ind) {
    ...
    // auto &cur_in_port =
    // _port_interf.in_port(_soft_config.in_ports_active[i]);
