...
struct StreamExecutor : dsa::sim::stream::Functor {
  ...
  void Visit(ConstPortStream *cps) override {
    ...
    while (stream.stream_active() && pushed < accel->get_ssim()->spec.const_bandwidth) {
      ...
      for (auto &elem : stream.pes) {
        ...
        in.push(data, sim::stream::AffineStatus());
      }
  ...
  void Visit(LinearReadStream *lrs) override {
    ...
    if (!bufferAvailable(lrs->pes, cacheline)) {
      return;
    }
    ...
    int to_reserve = std::accumulate(info.mask.begin(), info.mask.end(), (int) 0, std::plus<int>());
    reserveBuffers(lrs->pes, to_reserve);
  ...
  void Visit(LinearWriteStream *lws) override {
    ...
    auto &ovp = out_port;
    int port_available = ovp.scalarSizeInBytes() * ovp.vectorLanes();
   ...
   */
  bool bufferAvailable(const std::vector<PortExecState> &pes, int bytes) {
    for (auto elem : pes) {
      if (!accel->input_ports[elem.port].canPush(bytes)) {
        return false;
      }
    }
    return true;
   ...
   */
  bool reserveBuffers(const std::vector<PortExecState> &pes, int bytes) {
    for (auto elem : pes) {
      accel->input_ports[elem.port].ongoing += bytes;
    }
    return true;
  }
  ...
  void Visit(RecurrentStream *pps) override {
    ...
    while (pps->stream_active() && ovp.canPop(pps->dtype)) {
      if (!bufferAvailable(pps->pes, pps->dtype)) {
        break;
      }
      ...
      for (auto &elem : pps->pes) {
        ...
        ivp.push(data, sim::stream::AffineStatus());
        DSA_LOG(RECUR)
...
struct SPADResponser : dsa::sim::stream::Functor {
  ...
  void VisitImpl(StreamT *stream) {
    ...
    for (auto &elem : stream->pes) {
      auto &ivp = accel->input_ports[elem.port];
      ivp.push(data, response.info.as);
  ...
  if (Minor::LSQ::LSQRequestPtr response =
    ...
    if (_accel->_accel_index == response->sdInfo->which_accel) {
      ...
      if (port_in_okay) {
        ...
        for (int in_port : response->sdInfo->ports) {
          auto &in_vp = _accel->input_ports[in_port];
          in_vp.push(data, response->sdInfo->as);
          if (last) {
...
bool accel_t::done(bool show, int mask) {
  ...
  #define CHECK_PORTS(io, vec, buffer)                                                          \
    for (auto &elem : bsw.ports[io]) {                                                          \
      auto &port = vec[elem.port];                                                              \
      if (port.stream && (port.stream->barrier_mask() & mask)) {                                \
        DSA_LOG(WAIT) << IO_STR[io] << "Port: " << port.id() << " " << port.stream->toString(); \
        return false;                                                                           \
      }                                                                                         \
      if (mask == -1) {                                                                         \
        if (!port.buffer.empty()) {                                                             \
          DSA_LOG(WAIT) << IO_STR[io] << "Port: " << port.id() << " data entry not empty!";     \
          return false;                                                                         \
        }                                                                                       \
      }                                                                                         \
    }
  CHECK_PORTS(0, input_ports, buffer);
  CHECK_PORTS(1, output_ports, raw);
  #undef CHECK_PORTS
  return true;
}
